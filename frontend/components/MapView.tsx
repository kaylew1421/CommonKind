...
import type { Hub } from "../../types"; // ← use shared Hub type
...
function FlyTo({ center }: { center: [number, number] }) {
  ...
  useEffect(() => {
    ...
    if (dist > 0.0005) {
      map.flyTo(center as LatLngExpression, Math.max(map.getZoom(), 12), {
        animate: true,
        duration: 0.5,
      });
      last.current = center;
...
const MapView: React.FC<Props> = ({
  ...
  const highlightedHub = useMemo(
    () => hubs.find((h) => h.id === highlightedHubId) ?? null,
  ...
  const markers = useMemo(
    () =>
      hubs.map((h) => ({
        ...h,
        pos: [h.lat, h.lng] as [number, number],
        highlighted: h.id === highlightedHubId,
      })),
    [hubs, highlightedHubId]
  );
          ...
          <Popup>
            <div className="text-sm">
              ...
              <button
                ...
                className="mt-2 px-2 py-1 text-xs rounded bg-teal-600 text-white"
              >
                View details
              </button>
            </div>
